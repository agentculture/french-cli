...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope french-cli
...
scope:** the default personal scope is **private** (`--scope french-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `french-cli`). Private records are served only to a recall in the same
