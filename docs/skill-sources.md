...
french-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. french-cli
...
#   - consumer-identifying prose: `guildmaster` → `french-cli` (NOT
...
If a re-sync would lose a french-cli adaptation, lift the change
...
#     → `which french-cli vendors from guildmaster`
