...
from french.cli._errors import EXIT_ENV_ERROR, CliError
...
def add_learner(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        "--learner",
        help="Learner id (human or agent). Default: $FRENCH_CLI_LEARNER or the OS user.",
    )
...
def load_state(learner: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise CliError(
            code=EXIT_ENV_ERROR,
            message=str(exc),
            remediation="fix or remove the state file under $FRENCH_CLI_LEARN_HOME "
            "(or $XDG_DATA_HOME/french_cli/learn)",
        ) from exc
