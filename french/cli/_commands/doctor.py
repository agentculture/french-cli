"""``french doctor`` — self-check + contract pin (contract: subject_doctor).
...
from french.cli._commands.whoami import find_culture_yaml, read_agent_fields
from french.cli._output import emit_result
from french.contract_cite import CONTRACT_VERSION, SCHEMA_NAMES, list_schemas
from french.tutor import state, stories, subject
...
def _state_writable_check() -> dict[str, object]:
    ...
    try:
        ...
        remediation = "set $FRENCH_CLI_LEARN_HOME to a writable directory"
    return {
...
def _contract_pin_check() -> dict[str, object]:
    ...
    return {
        ...
        "remediation": "" if passed else "re-sync french/contract_cite from learn-cli",
    }
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    if json_mode:
        ...
        lines = [f"french-cli doctor: {status} (contract {CONTRACT_VERSION})", ""]
