"""``french story`` — the shared content surface (list + read).
...
Bare ``french story`` defaults to ``story list``.
"""
...
from french.tutor import curriculum, engine, state, stories, subject
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "story",
        help="Read graded stories (see 'french story list').",
    )
    _tutor.add_json(p)
