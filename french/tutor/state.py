...
* **XDG path resolution** — ``$FRENCH_CLI_LEARN_HOME`` >
...
Nothing here is French-specific: the store keys off ``subject.SUBJECT_ID`` and
...
from french.contract_cite import MASTERY_LEVELS, RESULTS
from french.tutor import subject
...
def resolve_learner(explicit: str | None = None) -> str:
    """Learner id: ``--learner`` > ``$FRENCH_CLI_LEARNER`` > OS user > ``default``.
...
def state_dir() -> Path:
    """Data dir: ``$FRENCH_CLI_LEARN_HOME`` > ``$XDG_DATA_HOME/french_cli/learn``
    > ``~/.local/share/french_cli/learn``."""
    override = os.environ.get(_HOME_ENV)
    if override:
        return Path(override).expanduser()
    xdg = os.environ.get("XDG_DATA_HOME")
    base = Path(xdg).expanduser() if xdg else Path.home() / ".local" / "share"
    return base / "french_cli" / "learn"
