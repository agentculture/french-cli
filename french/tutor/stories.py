...
/ ``story read`` payload bodies. It is subject-agnostic — the French-ness lives
entirely in the JSON files.

Content-dir resolution (first that exists wins):

1. ``$FRENCH_CLI_CONTENT_DIR`` — explicit override (tests, alternate content);
2. the repo-root ``content/stories/`` walking up from this file (source checkout);
3. the packaged ``french/_content/stories/`` (the wheel force-includes it).
"""
...
from french.contract_cite import validate

_CONTENT_ENV = "FRENCH_CLI_CONTENT_DIR"
