...
against the cited contract schema (``french/contract_cite``). It also checks the
...
from french.contract_cite import CONTRACT_VERSION, validate
...
def test_lesson_start_conforms(capsys: pytest.CaptureFixture[str]) -> None:
    rc, payload = run_json(
        capsys, ["lesson", "start", "l.greetings", "--learner", LEARNER, "--json"]
    )
    assert rc == 0
...
def test_lesson_repeat_harder_conforms(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    capsys.readouterr()
    rc, payload = run_json(
        capsys, ["lesson", "repeat", "l.numbers", "--harder", "--learner", LEARNER, "--json"]
    )
    assert rc == 0
    _valid(payload, "lesson")
...
def test_practice_scoped_conforms(capsys: pytest.CaptureFixture[str]) -> None:
    rc, payload = run_json(capsys, ["practice", "fr.numbers.prix", "--learner", LEARNER, "--json"])
    assert rc == 0
    _valid(payload, "practice")
    assert payload["scope"] == "fr.numbers.prix"
...
def test_record_conforms(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["recorded"]["item_id"] == "fr.numbers.prix"
...
def test_unknown_story_is_user_error(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["story", "read", "fr-does-not-exist", "--json"])
