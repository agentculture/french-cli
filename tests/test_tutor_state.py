...
from french.tutor import state

# --- path resolution --------------------------------------------------------


def test_home_env_wins(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("FRENCH_CLI_LEARN_HOME", str(tmp_path / "explicit"))
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path / "xdg"))
    assert state.state_dir() == tmp_path / "explicit"


def test_xdg_fallback(monkeypatch, tmp_path) -> None:
    monkeypatch.delenv("FRENCH_CLI_LEARN_HOME", raising=False)
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path / "xdg"))
    assert state.state_dir() == tmp_path / "xdg" / "french_cli" / "learn"
...
def test_home_default(monkeypatch, tmp_path) -> None:
    ...
    assert state.state_dir() == tmp_path / ".local" / "share" / "french_cli" / "learn"


def test_learner_resolution_precedence(monkeypatch) -> None:
    monkeypatch.setenv("FRENCH_CLI_LEARNER", "env-user")
...
def test_save_load_round_trip() -> None:
    ...
    state.record_result(st, item_id="fr.greetings.bonjour", activity="lesson", result="pass")
    state.set_current(st, "premiers-pas", "fr.greetings.bonjour")
    path = state.save(st)
    assert path.is_file()

    reloaded = state.load("ori")
    assert reloaded["learner"] == "ori"
    assert reloaded["mastery"]["fr.greetings.bonjour"] == "mastered"
    assert reloaded["current"] == {"module_id": "premiers-pas", "item_id": "fr.greetings.bonjour"}
    assert len(reloaded["history"]) == 1


def test_resume_across_sessions() -> None:
    st = state.load("resumer")  # fresh
    state.record_result(st, item_id="fr.numbers.prix", activity="practice", result="partial")
    state.save(st)
    # A brand new load (new "session") sees the persisted mastery.
    again = state.load("resumer")
    assert again["mastery"]["fr.numbers.prix"] == "practiced"
...
def test_repeat_difficulty_bumps() -> None:
    ...
    assert state.repeat_difficulty(st, "l.numbers") == 1
    assert state.bump_repeat(st, "l.numbers") == 2
    assert state.repeat_difficulty(st, "l.numbers") == 2
